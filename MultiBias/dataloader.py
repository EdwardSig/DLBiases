...
class MLImplicitBCELossDataLoader(BaseImplicitBCELossDataLoader):
    def __init__(self, dataset_path: str, file_name: tuple, device: torch.device, has_item_pool_file: bool = False):
        ...
        if len(file_name) >= 3:
            self.tmp_train_path: str = self.train_data_path
            self.train_sel_path: str = os.path.join(self.dataset_path, file_name[2])
            train_sel_df: pd.DataFrame = pd.read_csv(self.train_sel_path)
            self.train_df = pd.concat([self.train_df, train_sel_df])
            self.train_data_path: str = os.path.join(self.dataset_path, file_name[3])
            self.train_df.to_csv(self.train_data_path, index=False)
